...
public class ClientDemo extends javax.swing.JFrame {
	...
	private void initComponents() {
		...
		jTextFieldIPAddress.setText("19.122.88.61");

		jLabelPortNumber.setText("端口");

		jLabelPassWord.setText("密码");

		jPasswordFieldPassword.setText("a12345678");
	...
	/*************************************************
	 * 函数: "预览" 按钮单击响应函数 函数描述: 获取通道号,打开播放窗口,开始此通道的预览
	 *************************************************/
	...
	class FRealDataCallBack implements HCNetSDK.FRealDataCallBack_V30 {
		...
		String command = "ffmpeg -y -i pipe:0 -c:v libx264 -s 704x576 -tune zerolatency -crf 28 -g 25 -r 25 -preset ultrafast -f hls -hls_time 2.0 -hls_list_size 3 -hls_delete_threshold 5 -hls_flags delete_segments+temp_file E:/rtmp/hls/218bce00ebdbae83e0a1df66066ef501/index.m3u8";

		public boolean IS_WINDOW = System.getProperty("os.name").toLowerCase().startsWith("win");

		Process process = null;

		OutputStream outputStream;

		String[] buildCommandArray(String command) {
			if (IS_WINDOW) {
		...
		public void invoke(NativeLong lRealHandle, int dwDataType, ByteByReference pBuffer, int dwBufSize,
			...
			try {
				if (Objects.isNull(process) || !process.isAlive()) {
					ProcessBuilder builder = new ProcessBuilder(buildCommandArray(command));
				...
				case HCNetSDK.NET_DVR_STREAMDATA: // 码流数据
					if ((dwBufSize > 0) && (m_lPort.getValue().intValue() != -1)) {
						...
						byte[] byteArray = pointer.getByteArray(0, dwBufSize);
...
//						}
		...
		public void killProcess() {
			...
			if (pid == -1) {
				...
				if (Platform.isWindows()) {
					command = "taskkill /t /f /pid  " + pid;
				} else if (Platform.isLinux()) {
					command = "pkill -P " + pid;
				}
				try {
					new ProcessBuilder(buildCommandArray(command)).start();
				} catch (IOException e) {
					e.printStackTrace();
				}
			}
	...
	class NVRCallbackHardware implements HCNetSDK.FPlayDataCallBack {
		...
		public NVRCallbackHardware(HCNetSDK hcNetInter, NativeLong lPlayHandle) {
		...
		public void invoke(NativeLong lRealHandle, int dwDataType, ByteByReference pBuffer, int dwBufSize,
			...
			System.out.println("int dwBufSize, Pointer pUser");
			/**
		...
		public void invoke(NativeLong lPlayHandle, int dwDataType, ByteByReference pBuffer, int dwBufSize, int dwUser) {
			System.out.println("int dwBufSize, int dwUser");
		}
	}
}
